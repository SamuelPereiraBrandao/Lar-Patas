...
async function submit() {
    ...
    if (!response.ok) {
        error.value = data.message || "Não foi possível redefinir a senha.";
        return;
    }
        ...
        <v-card
            ...
            ><div class="auth-banner pa-7 text-center">
                <div class="auth-icon mx-auto mb-4">
                    <v-icon icon="mdi-shield-key-outline" size="29" />
                </div>
                ...
                <p class="text-body-2 mt-3 mb-0">
                    Use ao menos 8 caracteres para proteger sua conta.
                </p>
            </div>
            <v-card-text class="pa-6 pa-md-8"
                ...
                ><v-form @submit.prevent="submit"
                    ><v-text-field
                        ...
                        block
                        rounded="lg"
                        >Salvar nova senha<v-icon
                            end
                            icon="mdi-check" /></v-btn></v-form></v-card-text
        ></v-card>
    </div>
</template>
