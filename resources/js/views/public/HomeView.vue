...
import PendingActions from "../../components/layout/PendingActions.vue";
...
import PostOptions from "../account/profile/PostOptions.vue";
...
async function loadFeed(page = 1) {
    const response = await fetch(`/api/community/feed?page=${page}`, {
...
async function openComposer() {
    ...
    await nextTick();
    composerSection.value?.scrollIntoView({
        behavior: "smooth",
        block: "center",
    });
}
...
async function comment(post, body) {
    ...
    try {
        ...
        if (!response.ok)
            return notify(
                data.message || "Não foi possível comentar.",
                "error",
            );
        post.comments ||= [];
...
<template>
        ...
        <v-container class="py-10 py-md-13">
            <v-row align="center">
                <v-col cols="12" md="8" lg="7">
                    <v-chip
                        ...
                        ><v-icon
                            ...
                            color="primary"
                        />COMUNIDADE LAR & PATAS</v-chip
                    >
                    <h1 class="text-h3 text-md-h2 font-weight-black mb-4">
                        Pets, histórias e pessoas que cuidam.
                    </h1>
                    <p class="hero-copy text-h6 mb-7">
                        Compartilhe momentos com seu pet, conheça novas pessoas
                        e encontre um amigo que está esperando por um lar.
                    </p>
                    <div class="d-flex flex-wrap ga-3">
                        <v-btn
                            ...
                            size="large"
                            rounded="lg"
                            >Entrar na comunidade<v-icon end icon="mdi-plus"
                        /></v-btn>
                    </div>
                </v-col>
                <v-col
                ...
                >
                    <v-card
                        to="/perfil"
                        class="hero-community-card pa-5"
                        rounded="xl"
                        elevation="0"
                    >
                        <div class="d-flex align-center ga-3 mb-4">
                            ...
                            <div>
                                ...
                                <div class="text-caption opacity-70">
                                    Seu espaço para viver a vida com pets.
                                </div>
                            </div>
                        </div>
                        <div class="hero-stat-grid">
                            ...
                            <div>
                                <b>{{ store.pets.length }}</b
                                ><span>pets procurando lar</span>
                            </div>
                        </div>
                    </v-card>
                </v-col>
        ...
        <v-row align="start">
            <v-col cols="12" lg="8">
                <div class="d-flex align-center justify-space-between mb-5">
                    <div>
                        <div class="section-kicker mb-1">COMUNIDADE</div>
                        <h2 class="text-h4 font-weight-bold">
                            Histórias recentes
                        </h2>
                        <p class="text-medium-emphasis mt-1">
                            Veja o que as pessoas e seus pets estão vivendo.
                        </p>
                    </div>
                    <v-btn
                        v-if="isLogged"
                ...
                <div v-else class="feed-list">
                    ...
                    >
                        <v-card-item class="pb-2"
                            ><template #append
                                ><PostOptions
                                    v-if="
                                        isLogged &&
                                        composerAuthor.id === post.user_id
                                    "
                                    :post="post"
                                    @changed="loadFeed()" /></template
                            ><template #prepend
                                ><v-menu
                                    open-on-hover
                                    :open-on-click="false"
                                    ...
                                    @update:model-value="
                                        (open) =>
                                            open &&
                                            loadProfileSummary(post.user?.id)
                                    "
                            ...
                            ><v-menu
                                ...
                                ><template #activator="{ props: menuProps }"
                                    ><v-card-title
                                        class="px-0 text-body-1 font-weight-bold cursor-pointer"
                                        v-bind="menuProps"
                                        @click.stop="
                                            $router.push(
                                                `/perfil/${post.user.id}`,
                            ...
                            ><v-card-subtitle class="px-0"
                                >{{
                                    post.user?.city || "Comunidade Lar & Patas"
                                }}
                                ·
                                {{
                        ...
                        <v-card-text
                            v-if="post.body"
                            class="pt-1 text-body-1 post-body"
                        ...
                        <v-carousel
                            ...
                            :show-arrows="
                                post.gallery_urls.length > 1 ? 'hover' : false
                            "
                            ><v-carousel-item
                        ...
                        <v-card-actions class="px-4 py-3"
                            ...
                            ><v-spacer /><v-btn
                                ...
                                >{{ post.likes_count || 0 }}</v-btn
                            ><span class="text-caption text-medium-emphasis"
                                ><v-icon size="16" icon="mdi-comment-outline" />
                                {{ post.comments_count || 0 }} comentários</span
                            ><v-btn
                                :to="`/perfil/${post.user.id}`"
                                color="primary"
                                variant="text"
                    ...
                    <v-card
                        v-if="!posts.length"
                        class="feed-empty pa-8 text-center"
                        rounded="xl"
                        elevation="0"
                        ><v-icon
                            ...
                            color="primary"
                        />
                        <h3 class="mt-3">A comunidade está começando</h3>
                        <p class="text-medium-emphasis mt-1">
                        ...
                        <v-btn
                            ...
                            class="mt-3"
                            @click="openComposer"
                            >Criar publicação</v-btn
                        ></v-card
                    >
                    <div ref="feedEnd" class="text-center py-5">
                        <v-progress-circular
                        ...
                        /><span
                            ...
                            >Você viu todas as publicações por enquanto.</span
                        >
                    </div>
                </div>
            </v-col>
            <v-col cols="12" lg="4" id="adocao"
                ><aside class="adoption-sidebar">
                    <v-card
                        class="adoption-callout pa-6 mb-5"
                        rounded="xl"
                        elevation="0"
                        ><v-icon
                            ...
                            color="secondary"
                            size="32" />
                        <h2 class="text-h5 font-weight-bold mt-3">
                            Também é lugar de adotar
                        </h2>
                        <p class="text-medium-emphasis mt-2">
                            Conheça pets que esperam uma família e acompanhe
                            seus interesses.
                        </p>
                        <v-btn
                            to="/pets"
                            ...
                            class="mt-4"
                            rounded="lg"
                            >Explorar adoções<v-icon
                                end
                                icon="mdi-paw" /></v-btn
                    ></v-card>
                    <div
                        class="d-flex align-center justify-space-between mb-3 px-1"
                    >
                        <h3 class="text-h6 font-weight-bold">
                            Pets em destaque
                        </h3>
                        <v-chip size="x-small" color="primary" variant="tonal"
                            >{{ store.pets.length }} disponíveis</v-chip
                        >
                    </div>
                    <v-card
                        ...
                        elevation="0"
                        :to="`/pets/${pet.id}`"
                        ><v-avatar rounded="lg" size="68" color="primary"
                            ><v-img
                                v-if="pet.image_url"
                                :src="pet.image_url"
                                cover /><v-icon v-else icon="mdi-paw"
    ...
    <section class="adoption-strip">
        <v-container class="py-10 py-md-12"
            ><div
                class="d-flex flex-wrap align-end justify-space-between ga-3 mb-5"
            >
                <div>
                    <div class="section-kicker mb-1">ADOÇÃO RESPONSÁVEL</div>
                    <h2 class="text-h4 font-weight-bold">
                        Encontre seu próximo melhor amigo
                    </h2>
                </div>
                <v-btn to="/pets" color="primary" variant="outlined"
                    >Ver todos os pets</v-btn
                >
            </div>
            <v-row
                ><v-col
...
<style scoped>
.community-hero {
    background: linear-gradient(110deg, #083e39 0%, #0d766d 56%, #129487 100%);
}
.hero-copy {
    max-width: 680px;
    color: #d9f2ed;
