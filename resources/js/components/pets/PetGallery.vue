...
<template>
  <v-carousel :height="height" show-arrows="hover" hide-delimiter-background rounded="xl" class="pet-gallery">
    <v-carousel-item v-for="(photo, index) in photos" :key="photo" :src="photo" cover>
      <div class="gallery-caption">Foto {{ index + 1 }} de {{ photos.length }}</div>
    </v-carousel-item>
  </v-carousel>
</template>

<style scoped>
.pet-gallery { border-radius: 24px !important; overflow: hidden; }
.pet-gallery :deep(.v-window__container), .pet-gallery :deep(.v-carousel-item), .pet-gallery :deep(.v-img) { border-radius: inherit; }
.gallery-caption { position: absolute; right: 16px; bottom: 16px; padding: 6px 10px; border-radius: 999px; background: rgba(8, 22, 19, .72); color: white; font-size: .78rem; }
</style>
