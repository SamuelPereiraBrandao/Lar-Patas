...
const page = ref(1);
const applicantPages = ref({});
const applicantsPerPage = 5;
function applicantPage(pet) {
    return Math.min(
        applicantPages.value[pet.id] || 1,
        Math.max(1, Math.ceil(pet.adoptions.length / applicantsPerPage)),
    );
}
function visibleApplicants(pet) {
    const start = (applicantPage(pet) - 1) * applicantsPerPage;
    return pet.adoptions.slice(start, start + applicantsPerPage);
}
...
function adoptionLabel(adoption) {
    if (adoption.cancelled_at) return "Cancelada";
    if (adoption.released_at) return "Adoção concluída";
    if (adoption.status === "approved" && adoption.pickup_at)
        return "Aguardando liberação";
    return (
        {
            pending: "Interesse recebido",
            approved: "Aprovado",
...
function pickupDate(adoption) {
    return new Date(adoption.pickup_at).toLocaleString("pt-BR", {
        dateStyle: "short",
        timeStyle: "short",
        timeZone: adoption.pickup_timezone || "America/Sao_Paulo",
    });
}
const headers = [
    { title: "Pet", key: "name" },
        ...
        .filter((pet) => {
            ...
            return (
                ...
                (!filters.value.status ||
                    pet.status === filters.value.status) &&
                (!filters.value.species ||
                    pet.species === filters.value.species) &&
                (!filters.value.shelter ||
                    (filters.value.shelter === "none"
                        ? !pet.shelter_id
                        : pet.shelter_id === filters.value.shelter))
            );
        })
        .sort(
            (a, b) =>
                        ...
                        <td :colspan="columns.length" class="pa-0">
                            <div class="pa-5 expanded-pet">
                                <div class="d-flex align-center ga-3 mb-4">
                                    ...
                                    />
                                    <div>
                                        <b>Um novo lar para {{ item.name }}</b>
                                        <p
                                            class="text-caption text-medium-emphasis"
                                        >
                                            Conheça os interessados e acompanhe
                                            a retirada.
                                        </p>
                                    </div>
                                </div>
                                <div class="applicants-list">
                                    <v-card
                                        v-for="a in visibleApplicants(item)"
                                        :key="a.id"
                                        rounded="lg"
                                        elevation="0"
                                        class="applicant-card pa-4"
                                    >
                                        <div class="d-flex align-center ga-3">
                                            <v-avatar size="56" color="primary"
                                                ><v-img
                                                    v-if="a.user?.avatar_url"
                                        ...
                                        <div class="applicant-actions">
                                            <v-chip
                                                ...
                                                size="small"
                                                variant="tonal"
                                                >{{ adoptionLabel(a) }}</v-chip
                                            >
                                            <v-btn
                                                v-if="canSchedule(item, a)"
                                                color="primary"
                                                variant="tonal"
                                                rounded="lg"
                                                prepend-icon="mdi-home-heart"
                                                @click="openPickup(item, a)"
                                                >Aprovar adoção</v-btn
                                            >
                                            <v-btn
                                                v-if="
                                                    a.status === 'approved' &&
                                                ...
                                                @click="
                                                    openPickup(item, a, true)
                                                "
                                                >Validar código e liberar</v-btn
                                            >
                                        </div>
                                        <div
                                            v-if="a.pickup_at"
                                            class="applicant-pickup text-body-2"
                                        >
                                            <v-icon
                                    ...
                                    >
                                        {{
                                            ...
                                            Math.min(
                                                applicantPage(item) *
                                                    applicantsPerPage,
                                                item.adoptions.length,
                                            )
                                        }}
                                        de
                                        {{ item.adoptions.length }} interessados
                                    </span>
                                    <v-pagination
                                        v-if="
                                            item.adoptions.length >
                                            applicantsPerPage
                                        "
                                        :model-value="applicantPage(item)"
                                        :length="
                                            Math.ceil(
                                                item.adoptions.length /
                                                    applicantsPerPage,
                                        ...
                                        "
                                        :total-visible="3"
                                        :aria-label="`Páginas de interessados em ${item.name}`"
                                        size="small"
                                        color="primary"
                                        rounded="lg"
                                        @update:model-value="
                                            applicantPages[item.id] = $event
                                        "
                                    />
                                </div>
                                <p
                                ...
                                >
                                    Nenhuma demonstração de interesse ainda.
                                </p>
                            </div>
                        </td>
                    </tr></template
        ...
        <v-dialog v-model="filterDialog" max-width="600" scrollable>
            <v-card rounded="xl">
                <div class="d-flex align-center ga-3 pa-6">
                    ...
                    /></v-avatar>
                    <div>
                        <h2 class="text-h6 font-weight-bold">Filtrar pets</h2>
                        <p class="text-body-2 text-medium-emphasis">
                            Encontre quem você está procurando.
                        </p>
                    </div>
                    <v-spacer /><v-btn
                        icon="mdi-close"
                ...
                <v-card-text class="pa-6">
                    ...
                    <p class="text-caption text-medium-emphasis mt-2 mb-6">
                        Disponíveis primeiro, depois em processo e adotados.
                    </p>
                    <v-row>
                        <v-col cols="12" sm="6">
                            <v-select
                                v-model="draftFilters.species"
                                :items="[
                                    { title: 'Todas as espécies', value: null },
                                    { title: 'Cachorros', value: 'dog' },
                        ...
                        <v-col cols="12" sm="6">
                            <v-select
                                v-model="draftFilters.shelter"
                                :items="[
                                    { title: 'Todas as sedes', value: null },
                                    { title: 'Sem sede', value: 'none' },
                                    ...shelterOptions,
                                ]"
                                label="Sede"
                                variant="outlined"
